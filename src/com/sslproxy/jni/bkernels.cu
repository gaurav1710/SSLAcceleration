...
inline void parallel_mult(radix_type *m, radix_type *n, int k, radix_type *res,
		int stridem, int striden, int num_req, int streamid) {
	//THREADSPERBLOCK[0]
	int streams_req =  1;
...
inline void parallel_add(radix_type *a, radix_type *b, int k, int num_req,
		int stridea, int strideb, radix_type *residue_carry, int streamid) {
	//THREADSPERBLOCK[1]
...
inline void montgomery(radix_type *a, radix_type *b, int k, radix_type *m,
	...
	//T=a*b
	parallel_mult(a, b, k, T, k, k, num_req, tid);
	...
	//M=T*m'
	parallel_mult(T, mbar, k, M, 2 * k, k, num_req, tid);
	if (DEBUG) {
	...
	//U=M*m
	parallel_mult(M, m, k, U, 2 * k, k, num_req, tid);
	if (DEBUG) {
	...
	//U=T+M*m
	parallel_add(U, T, 2 * k, num_req, 2 * k, 2 * k, addition_carry, tid);
	if (DEBUG) {
	...
	parallel_sub(carry, m, k, res, addition_carry, num_req, 2 * k, k, 1, tid);
	if (DEBUG) {
...
void calmerge_m1_m2(radix_type *c, radix_type *e, radix_type *m, int k,
		...
		radix_type *qinv, radix_type *res, int num_req, int tid, int base) {
	...
	hmod_exp_sqmul(c, e, k, m, M, rinv, mbar, r2modm, 2 * num_req, tid, base);
	...
	//(M1-M2)*(qinv mod p)*q
	parallel_mult(mont, q, k, t2, k, k, num_req, tid);
	if (DEBUG) {
