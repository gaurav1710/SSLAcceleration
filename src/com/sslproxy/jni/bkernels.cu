...
void print_arr(radix_type *a, int n, char label[], int gcopy){
	if(gcopy){
		radix_type *ha = (radix_type *)malloc(n*sizeof(radix_type));
		copy_data_to_host(ha,a,n,0);
...
for (int i = 0; i < num_req; i++) {
	...
	execute(req);
	if(DEBUG){
		print_arr(req->m1m2,num_req*2*k ,"Res", 0);
	}
	for(int i=0;i<num_req;i++){
